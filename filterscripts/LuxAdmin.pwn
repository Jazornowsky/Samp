...
public OnPlayerRequestSpawn(playerid)
{
	...
	SendClientMessage(playerid,lightred,"SERVER: Your account isn't registered. Please Register! | "" [Password]");
	return 0;
	}
	// Request Login
...
dcmd_level(playerid,params[])
{
    if(AccInfo[playerid][Level] >= 1)
	{
		...
		SendClientMessage(playerid,0x33F666FF,"Adm: saveskin, useskin, dontuseskin, setmytime, adminarea");
		SendClientMessage(playerid,0x53D212FF,"Other: lconfig, morning, reports, richlist, miniguns");
		}
		...
		SendClientMessage(playerid,0x53D212FF,"Other: gps, setpass, lammo, countdown, aweaps, invisible, visible");
		}
...
dcmd_countdown(playerid,params[])
{
    ...
    if(AccInfo[playerid][Level] >= 3)
	{
        if(CountDown == -1)
		{
			...
			return SendCommandToAdmins(playerid,"CountDown");
		}
		else return SendClientMessage(playerid,red,"ERROR: Already have a Countdown in Progress!");
	}
	else return ErrorMessages(playerid, 1);
}
...
dcmd_lcredits(playerid,params[])
 {
	...
	SendClientMessage(playerid, orange, " Created by: LuxurioN");
...
stock CheckAdmin(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_TYPE_RCON2, DIALOG_STYLE_INPUT,
	"LuxAdmin - RCON!",string, "Enter", "Exit");
	return  1;
}
#endif
//==============================================================================
